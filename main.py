...
with st.sidebar:
    st.title("Settings")
    GROQ_API = st.text_input("Enter your Groq API:",type="password")
    model_name = st.selectbox("Which model do you want?",
                              ("deepseek-r1-distill-llama-70b","llama-3.3-70b-versatile","qwen-qwq-32b"),index=0)
...
if GROQ_API:
    ...
    loader = PyPDFLoader(file_path='hotelinfo.pdf')
    data = loader.load()
    ...
    system_prompt = (
            "You are a Chatbot for an Hotel answer all questions related to Hotel only the hotel name is: The Grand Horizon Hotel "
